...
import { FilePlus, Calendar, Link as LinkIcon, Users, AlertCircle } from 'lucide-react';
...
export default function CreateAssignmentModal({
  ...
  const [targetScope, setTargetScope] = useState('all'); // 'all' | 'specific'
  ...
  useEffect(() => {
    if (assignmentToEdit) {
      ...
      setSelectedGroupIds(assignmentToEdit.targets || []);
    } else {
      setTitle('');
      setDescription('');
      ...
      setOnedriveLink('https://onedrive.live.com/view/assignment-starter');
      setTargetScope('all');
      setSelectedGroupIds([]);
    }
    setError('');
  ...
  const handleSubmit = async (e) => {
    ...
    if (targetScope === 'specific' && selectedGroupIds.length === 0) {
    ...
    try {
      const payload = {
        ...
        target_group_ids: targetScope === 'specific' ? selectedGroupIds : undefined,
      };

      let result;
      if (isEditing) {
        result = await updateAssignmentApi(assignmentToEdit.id, payload);
      } else {
        result = await createAssignmentApi(payload);
      }
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <Input
            ...
            icon={LinkIcon}
            helperText="Optional reference or starter file link"
          />
        </div>
